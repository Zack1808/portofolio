...
const Navbar = ({ urls, switchToggle, setSwitchToggle, logo = "Logo", scrollTo }) => {
        ...
        <nav className='navbar'>
            ...
            <div className="navbar-container">
                ...
                <div className={toggle ? "navbar-link-list-active navbar-link-list" : "navbar-link-list"}>
                    ...
                    <Link to="/" className="navbar-link" style={{ transition: toggle && `opacity .5s ${n + .1}s`}} onClick={() => {setToggle(false); scrollTo()}}>Contact Me</Link>
                </div>
            </div>
        </nav>
    )
